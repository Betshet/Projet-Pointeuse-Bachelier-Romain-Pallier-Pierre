...
public class OngletDepartment extends JPanel implements ActionListener 
{
	private static final long serialVersionUID = 1L;

	private DefaultTableModel model;
	private JTable tableauEmp;
	private String[] nomColonnes = {"Nom","Manager", "Nombre d'employes","Nombre de managers"};
	...
	public OngletDepartment(Controller controller)
	{
		setControl(controller);
		
		setModel( new DefaultTableModel(getControl().getCompany().getAllDepartments(),nomColonnes) );
		...
		setChangeManagerButton(new JButton("Changer manager"));
		
		getBoutons().add(getAddButton());
		getBoutons().add(getRemoveButton());
		getBoutons().add(getChangeManagerButton());
		
		getAddButton().addActionListener(this);
		getRemoveButton().addActionListener(this);
		getChangeManagerButton().addActionListener(this);
	...
	public JButton getChangeManagerButton() {
		return changeManagerButton;
	}

	public void setChangeManagerButton(JButton changeManager) {
		this.changeManagerButton = changeManager;
	}
	...
	public void actionPerformed(ActionEvent arg0) 
	{
		if( arg0.getActionCommand().equals("Ajouter"))
			getControl().addRowDpt();
		if( arg0.getActionCommand().equals("Supprimer"))
			getControl().removeRowDpt();
		if( arg0.getActionCommand().equals("Changer manager"))
			getControl().changeManagerDpt();
	}
